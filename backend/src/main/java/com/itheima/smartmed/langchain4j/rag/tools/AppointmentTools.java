package com.itheima.smartmed.langchain4j.rag.tools;
...
public class AppointmentTools {
    ...
    public String bookAppointment(Appointment appointment){
        ...
        System.out.println("接收到的参数: " + appointment);

        //查找数据库中是否包含对应的预约记录
        Appointment appointmentDB = appointmentService.getOne(appointment);
        System.out.println("数据库查询结果: " + appointmentDB);

        if(appointmentDB == null){
            appointment.setId(null);//防止大模型幻觉设置了id
            boolean saved = appointmentService.save(appointment);
            System.out.println("保存结果: " + saved + ", 生成ID: " + appointment.getId());
            if(saved){
                return "预约成功，预约ID: " + appointment.getId();
            }else{
                return "预约失败";
            }
        }
